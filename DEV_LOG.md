...
### 10.8 — Fix remaining 10 Maestro failures: Android-specific + deep-link handler
**Time:** 2026-05-22  
**Type:** bug  
**Prompt:** Full 67-test suite run shows 57 pass / 10 fail — verify and fix remaining failures.  
**Activity:** Identified and fixed 5 distinct issues across 6 flows. (1) `feature-001-ac04`: `pressKey: Back` inside the onboarding enter-key step triggered hardware-back navigation, leaving the screen before `btn-submit` appeared — replaced with `hideKeyboard`. (2) `feature-009-ac04`: `assertVisible: "Breaking Bad"` was ambiguous — Maestro found the text both in the modal header and in the TV detail screen visible behind the overlay, causing a multi-match failure; removed the redundant assertion (modal confirmed by `episode-modal:btn-close`). (3) `feature-011-ac05`: Settings is a root stack screen (not a tab), so pressing Back from Settings returns to the Profile tab — added `pressKey: Back` + `tapOn: "My Lists"` to reach the My Lists tab before re-opening Inception. (4) `feature-012-ac01`: `assertVisible: "••••••••••••••••"` fails on Android — Maestro can't match U+2022 bullet characters (shows as `????????????????` in accessibility tree); replaced with `assertVisible: { id: "settings:btn-change-key" }`. (5) `feature-014-ac02`: `assertVisible: "Cancel"` + `tapOn: "Cancel"` is iOS-only (`UIActivityViewController` always shows Cancel); Android's share sheet has different UI with no Cancel button — replaced with `pressKey: Back` + `assertVisible: { id: "settings:btn-export" }` to confirm share sheet dismissed. Also added `watcho://import` handler in `app/_layout.tsx` so the `openLink: "watcho://import"` calls in feature-015 flows open the ImportModal (the existing handler silently dropped all non-`file://` URLs).  
**Outcome:** Commit `c999537` — all identified failures addressed; suite expected to reach 67/67.
